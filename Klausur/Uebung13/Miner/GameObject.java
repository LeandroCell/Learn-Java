 ...
 * dessen Ursprung am oberen Rand in der Mitte der Zeichenfläche ist.
 * x-Koordinaten wachsen nach unten rechts, y-Koordinaten nach unten links. Ein
 * Spielobjekt hat eine Rotation, die 0 in Richtung +x, 1 in Richtung +y, 2 in
 * Richtung -x und 3 in Richtung -y ist. Ein Spielobjekt hat außerdem eine
 * visuelle Erscheinung, die durch ein Bild vorgegeben ist. Dieses kann entweder
 * aus einer geladenen Grafikdatei stammen oder direkt gesetzt werden. Für das
 * Laden aus Dateien existiert die Konvention, dass abhängig von der Rotation
 * Dateien mit den entsprechenden Endungen "-0", "-1", "-2" und "-3" geladen
 * werden, wenn es sie gibt. Grafikdateien werden im Unterverzeichnis "images"
 * und Sound-Dateien im Unterverzeichnis "sounds" erwartet.
 *
 * @author Thomas Röfer
 */
class GameObject extends Game
{
    ...
    static {
        ...
        nextId = 1;
        new GameObject.Canvas(9, 10, 96, 48);
        int pos = 0;
        for (final Object tiles : new Object[] {
                "floor", "floor", "tree", "floor", "floor", "floor", "floor", "floor", "floor", "floor",
                "floor", "floor", "floor", "floor", "tree", "floor", "floor", "floor", "tree", "floor",
                "tree", "floor", "tree", "floor", "floor", "floor", "floor", "floor", "floor-e-0", "floor",
                "floor", "floor", "floor", "floor", "floor", "floor", "tree", "floor", "floor-i-0", "floor",
                "floor", "tree", "floor", "floor", "floor", "floor", "floor", "floor", "floor-i-0", "floor",
                "floor", "floor-l-0", "floor-i-1", "floor-i-1", new String[] {"floor-e-3", "goal"}, "floor", "floor", "floor", "floor-i-0", "floor",
                "floor", "floor-i-0", "floor", "tree", "floor", "floor", "floor", "floor", "floor-i-0", "floor",
                "floor", "floor-i-0", "floor", "floor", "floor", "floor", "tree", "floor", "floor-i-0", "floor",
                "tree", "floor-i-0", "floor", "floor", "floor", "floor", "floor", "floor", "floor-i-0", "floor",
                "floor", "floor-l-1", "floor-i-1", "floor-i-1", "floor-i-1", "floor-i-1", "floor-i-1", "floor-i-1", "floor-l-2", "floor",
            }) {
            for (final String tile : tiles instanceof String ? new String[] {(String) tiles} : (String[]) tiles) {
                new GameObject(pos / 10, pos % 10, 0, tile);
            }
            ++pos;
        }
    }
    ...
    static class Canvas extends JPanel
    {
        ...
        /**
         * Alle Spielobjekte, von hinten nach vorne geordnet und an gleicher
         * Tiefe in der Reihenfolge des Hinzufügens bzw. der letzten Bewegung.
         */
        private Set<GameObject> gameObjects = new TreeSet<>(
                (a, b) -> {
                    final double ax = a.getX() + a.getOffset() * offsets[a.getRotation()][0];
                    final double ay = a.getY() + a.getOffset() * offsets[a.getRotation()][1];
                    final double bx = b.getX() + b.getOffset() * offsets[b.getRotation()][0];
                    final double by = b.getY() + b.getOffset() * offsets[b.getRotation()][1];
                    final int aDepth = (int) Math.ceil(ax + ay);
                    final int bDepth = (int) Math.ceil(bx + by);
        ...
        private Rectangle getDrawRectangle(final GameObject gameObject)
        {
            ...
            final double xPos = gameObject.getX() + offsets[gameObject.getRotation()][0]
                    * gameObject.getOffset();
            final double yPos = gameObject.getY() + offsets[gameObject.getRotation()][1]
                    * gameObject.getOffset();
            final int x = xOrigin + (int) ((xPos - yPos)
                    * (cellWidth / 2) - (gameObject.getImage().getWidth() / 2));
            final int y = yOrigin + (int) ((xPos + yPos)
